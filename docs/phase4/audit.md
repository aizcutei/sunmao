...
| floating CLAP editor 的 set_transient | clap_rs/src/ext/gui.rs 默认 gui_set_transient 返回 false；SunmaoClapWrapper 的 GuiHandler 实现未 override；baseview 未提供 transient 所属窗口接线 | 未完成。需先核对上游 gui.h，再贯通格式层、统一 view 契约和原生窗口，覆盖 show 前设置与平台差异 |
| SpectrumAnalyzer/meter 消费 Phase 3 metering | sunmao/gui/src/widgets/spectrum.rs 只有 SpectrumSource/StaticSpectrum/SpectrumAnalyzer；widgets fixture 已新增 MeterSource，消费输出第一声道的 MeterHandle，显示 peak/RMS（-60..0 dBFS）；prelude/doc-test、数值属性、真实 process 到显示的零分配/reset 测试本地通过 | 实现待 hosted 验收，完整回归及打包检查进行中 |
...
下一瓶颈：M4 level meter 与 Phase 3 metering 接线；完成并通过独立三平台验收后，再处理 transient 契约。

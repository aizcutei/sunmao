...
| M5 Wayland 与总验收 | Wayland、GUI 侧兼容策略、proptest/文档收尾 | **未完成**：EGL 已验收；浮动窗口分派与帧循环已编译通过，真实 GL 编辑器运行验收已通过 #116，真实鼠标输入与像素更新已通过 #119；键盘/xkbcommon、修饰键与被动焦点已实现待 hosted 验收；cursor、主动 focus、output scaling 和 facade feature 传递未完成 | [run #119](https://github.com/aizcutei/sunmao/actions/runs/34216552175)（`7e78eb3`）三平台 success；日志确认真实鼠标输入与 shader 像素更新；三个 artifacts 已下载且 SHA-256、ZIP CRC 校验通过 | hosted 验证原生键盘，再完成 cursor/focus、缩放和 feature 传递 |
...
键盘/xkbcommon、修饰键与被动焦点已实现（待 hosted 真实按键验收）；
...
3. **baseview 原生 Wayland 编辑器已接通，输入与平台收尾仍在进行**：协议握手、EGL 和统一 GL 接口已由
   ...
   本轮键盘、compose、修饰键、被动焦点与 repeat 实现待 hosted 验收；cursor、主动 focus、缩放和 feature 传递仍待完成。

...
| M5 Wayland 与总验收 | Wayland、GUI 侧兼容策略、proptest/文档收尾 | **未完成**：EGL 已验收；浮动窗口分派与帧循环已编译通过，真实 GL 编辑器运行验收已通过 #116；输入和 output scaling 未完成 | [run #110](https://github.com/aizcutei/sunmao/actions/runs/34033340452)（`6977d6b`）三平台 success；日志确认 EGL 像素、resize、swap、teardown 测试通过；三个 artifacts 已下载且 SHA-256、ZIP CRC 校验通过 | 接入原生 Wayland 编辑器并由 hosted runner 验证 |
...
同一提交三平台全部 success。输入和 output scaling 仍未接入，因此 Phase 4 不能标记完成。
...
EGL 上下文与统一 GL 接口，均已验证；原生浮动窗口、事件循环与输入仍需接通。
...
3. **baseview 原生 Wayland 编辑器尚未接通**：协议握手、EGL 和统一 GL 接口已由
   run #110 验证；`Window::open_floating` 仍使用 X11，尚缺窗口事件循环和输入。

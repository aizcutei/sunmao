...
| M5 Wayland 与总验收 | Wayland、GUI 侧兼容策略、proptest/文档收尾 | **未完成**：EGL 已验收；浮动窗口分派与帧循环已编译通过，真实编辑器验收待运行；输入和 output scaling 未完成 | [run #110](https://github.com/aizcutei/sunmao/actions/runs/34033340452)（`6977d6b`）三平台 success；日志确认 EGL 像素、resize、swap、teardown 测试通过；三个 artifacts 已下载且 SHA-256、ZIP CRC 校验通过 | 接入原生 Wayland 编辑器并由 hosted runner 验证 |
...
### 当前判定：**Phase 4 未完成，M5 原生编辑器运行验收与输入待完成**
...
该测试尚待 hosted 运行；输入和 output scaling 仍未接入，因此 Phase 4 不能标记完成。

...
| M5 Wayland 与总验收 | Wayland、GUI 侧兼容策略、proptest/文档收尾 | **未完成**：EGL 已验收；浮动窗口分派与帧循环已编译通过，真实 GL 编辑器运行验收已通过 #116，真实鼠标输入与像素更新已通过 #119；键盘/xkbcommon、修饰键与被动焦点已由 #120 hosted 验收；cursor 通用协议、隐藏与原生双窗口重入的真实像素验收已由 8dee3e8 三平台 hosted 验证；主动 focus 已由 7ed7be0 三平台 hosted 验收；output scaling 和 facade feature 传递未完成 | [run #119](https://github.com/aizcutei/sunmao/actions/runs/34216552175)（`7e78eb3`）三平台 success，三份产物已下载且校验通过；[run #120](https://github.com/aizcutei/sunmao/actions/runs/34219433180)（`aa9d25e`）三平台 success，真实鼠标/键盘日志已核实，三份产物已下载且 SHA-256/ZIP CRC 校验通过；[光标验收](https://github.com/aizcutei/sunmao/actions/runs/34226222440)（8dee3e8）三平台 success，真实光标日志已核实，三份产物已下载且 SHA-256/ZIP CRC 校验通过 | output scaling 已实现且完整本地 gate 通过，待 Sway/Weston hosted 验收；随后 feature 传递 |
...
output scaling 与 facade feature 传递仍未完成，
因此 Phase 4 不能标记完成。

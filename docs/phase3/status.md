...
- M1 进行中：第 1、2 项均已验收。第 2 项（speaker layout 动态协商）hosted
  ...
  tail 上报 0 的真实缺陷。第 4 项（backend 层 expression/mod 端到端映射测试）本地
  就绪待 hosted，同样当即发现并修复了一个更严重的缺陷：**VST3 note expression 从未
  真正到达插件**（backend 在回调之后 clear 事件队列）。其余 3 项未开始。

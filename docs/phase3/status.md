...
| M3 DSP 基础组件 | `sunmao/dsp`：filters/envelopes/oscillators | 进行中：**filters 已落地待 hosted**（一阶/SVF/biquad + 4 proptest；SVF fixture 已换组件实现且**测试零改动**通过）。envelopes/oscillators 未开始 | 本地证据见 progress.md | envelopes（ADSR/follower）与 band-limited oscillators（sine/saw/pulse），随后用它们把 instrument 模板压进 50 行 |

...
- M1 进行中：第 1、2 项均已验收。第 2 项（speaker layout 动态协商）hosted
  ...
  `migrate_state` 因此永远不可能触发。第 6 项（preset 载入）本地就绪待 hosted：
  CLAP 侧完整落地，VST3 program list 按边界不实现（`vst3_sys` 无相应绑定，VST3
  宿主经 `setState` 走状态应用那一半），已在 semantics.md 记为单格式能力与降级。
  余下第 7 项（无界 fuzz 脚手架）未开始。

...
| M1 Phase 2 收口 | 7 项遗留（见 phase2/status.md 遗留表），每项完成即更新该表 | 进行中：**1/7 项已验收 + 1 项待 hosted**（第 1 项 bus 激活/去激活回调，全层落地 + 11 测试，三平台 hosted 全绿）。原盘点（2026-08-28）已确认 `_sys` 层无缺口（`clap_sys::clap_plugin_audio_ports_activation_t`、`audio_ports_config` 全家族、`vst3_sys::IComponent::activate_bus`），缺口在 `_rs`/core，activation 与 `audio_ports_config`（layout 协商）两侧现均已补齐 | 第 1 项：[run #42](https://github.com/aizcutei/sunmao/actions/runs/33171119003)（commit `b78aca6`）三 job success，全部 blocking 步骤零非成功，artifacts 齐备 | 第 3 项 runner 宿主侧断言（latency/tail 查询、多 bus 拓扑枚举、sidechain 送信号验证路由） |
...
- M1 进行中：第 2 项（speaker layout 动态协商）本地就绪，待 hosted；第 1 项
  （bus 激活/去激活回调）已验收——hosted run #42

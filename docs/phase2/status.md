...
| M2 latency/tail/render | 上报 + 变更通知 + render 模式 | 未开始；盘点：`clap_rs::Plugin` 已有 `latency()`/`tail()`/`set_render_mode()` 与 `ext/{latency,tail,render}.rs`，`vst3_rs::Plugin` 已有 `latency()`/`tail()`（`vst3_rs/src/plugin.rs:489`）。缺口：`sunmao_core::SunmaoPlugin` 无对应方法、两个 backend 未桥接、VST3 侧 `ProcessSetup.process_mode` 未向上暴露、runner 无 latency/tail 断言 | 盘点见本行 | core trait 加 `latency()`/`tail()`/`set_render_mode()`，两 backend 桥接，tempo delay fixture 上报 lookahead + tail |

...
| `clap.preset-load` / VST3 program list | M5 | **已落地，待 hosted 验收**（Phase 3 M1）：统一为 `SunmaoPlugin::load_preset(PresetLocation)` + `SUPPORTS_PRESET_LOAD`，CLAP 侧经 `clap.preset-load/2`（含 draft 别名）落地；**VST3 program list 按边界不实现**——`vst3_sys` 无 `IUnitInfo`/`IProgramListData` 绑定，VST3 宿主经 `setState` 走"状态应用"那一半，不存在静默丢事件。见 semantics.md 的"preset 载入"行 |

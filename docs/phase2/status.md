...
| backend 层 expression/mod 端到端映射测试 | M4 | **已落地，待 hosted 验收**（Phase 3 M1）：两格式各一个端到端测试（原始宿主事件走真实 `IEventList`/`clap_input_events_t` → core 队列）。**该测试当即发现 VST3 note expression 从未真正到达插件**（backend 在回调后 clear 队列），已修复并记入 semantics.md |

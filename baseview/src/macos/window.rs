...
use raw_window_handle::{
    ...
    RawDisplayHandle, RawWindowHandle, DisplayHandle, WindowHandle as RwhWindowHandle, HandleError,
};
...
impl<'a> Window<'a> {
    ...
    fn create_gl_context(ns_window: Option<id>, ns_view: id, config: GlConfig) -> GlContext {
        let ns_view_ptr = std::ptr::NonNull::new(ns_view as *mut c_void)
            .expect("ns_view is null");

...
use sunmao_gui::{
    GuiContext, Widget, ParameterWidget, Rect, Event, MouseButton, Modifiers,
    Knob, Slider, Button, Label, Orientation, Fill, Color,
};
use sunmao_gui_webview::{WebViewContext, generate_html_page};
...
fn main() {
    ...
    let mut button = Button::toggle("Bypass")
        .with_bounds(Rect::new(270.0, 40.0, 80.0, 28.0));
    
    let label = Label::new("SunMao Gain")
        .with_bounds(Rect::new(20.0, 100.0, 200.0, 24.0));
    
    // ========== Test Widget Interactions ==========
    println!("Testing Widget Interactions:");
    
    // Hover knob
    knob.handle_event(&Event::MouseMove { x: 52.0, y: 52.0, modifiers: Modifiers::none() });
    println!("  ✓ Knob hovered: {}", knob.state().hovered);
    
    // Scroll to change knob value
    knob.handle_event(&Event::Scroll { x: 52.0, y: 52.0, delta_x: 0.0, delta_y: 20.0, modifiers: Modifiers::none() });
    println!("  ✓ Knob value after scroll: {:.2}", knob.value());
    
    // Click button
    button.handle_event(&Event::MouseDown { x: 310.0, y: 54.0, button: MouseButton::Left, modifiers: Modifiers::none() });
    ...
    println!("  Generated {} JS drawing commands", ctx.get_commands().len());
    ...
    println!("\nOpen {} in a browser to see the WebView output.", html_path);
}

...
static DESCRIPTOR: SyncDescriptor = SyncDescriptor(clap_plugin_descriptor_t {
    ...
    id: b"com.sunmao.clap_sys.fx_gain\0".as_ptr() as *const c_char,
    name: b"Clap Sys Fx Gain\0".as_ptr() as *const c_char,
    ...
    description: b"Simple gain effect\0".as_ptr() as *const c_char,
...
unsafe extern "C" fn plugin_destroy(plugin: *const clap_plugin_t) {
    unsafe {
        let instance = (*plugin).plugin_data as *mut GainEffect;
        if !instance.is_null() {
...
unsafe fn apply_param_events(effect: &mut GainEffect, in_events: *const clap_input_events_t) {
    ...
    for index in 0..size {
            ...
            && (*header).size >= std::mem::size_of::<clap_event_param_value_t>() as u32
        {
            ...
            if event.param_id == PARAM_GAIN {
                effect.gain = event.value.clamp(0.0, 2.0);
            }
        }
    }
}
...
unsafe fn process_audio_f32(
    ...
    for ch in 0..channels {
        ...
        for i in 0..frames {
            out_buf[i] = in_buf[i] * effect.gain as f32;
        }
    }
}
...
unsafe fn process_audio_f64(
    ...
    for ch in 0..channels {
        ...
        let out_buf = std::slice::from_raw_parts_mut(out_ptr, frames);
        for i in 0..frames {
            out_buf[i] = in_buf[i] * effect.gain;
        }
    }
}
...
unsafe extern "C" fn plugin_process(
    ...
    let effect = &mut *((*plugin).plugin_data as *mut GainEffect);
    let process = &*process;
...
unsafe extern "C" fn params_get_value(
    ...
    let effect = &*((*plugin).plugin_data as *const GainEffect);
    *out_value = effect.gain;
    true
}
...
unsafe extern "C" fn state_save(
    ...
    let effect = &*((*plugin).plugin_data as *const GainEffect);
    let gain = effect.gain.to_le_bytes();
...
unsafe extern "C" fn state_load(
    ...
    let mut effect = &mut *((*plugin).plugin_data as *mut GainEffect);
    effect.gain = f64::from_le_bytes(buffer).clamp(0.0, 2.0);
    true
}
...
unsafe extern "C" fn create_plugin(
    ...
    let instance = Box::new(GainEffect { host, gain: 1.0 });

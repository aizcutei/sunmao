...
unsafe extern "system" fn processor_get_bus_count(
    ...
    if media_type == MediaTypes::kAudio
        && (dir == BusDirections::kInput || dir == BusDirections::kOutput)
    {
        1
    } else {
        0
...
unsafe extern "system" fn audio_set_processing(_this: *mut c_void, _state: TBool) -> tresult {
    kResultOk
}
...
impl GainGuiHandler {
    ...
    fn gain_value(&self) -> f64 {
        self.gain_state.get_gain().clamp(0.0, 2.0)
    }
    ...
    fn slider_rect(&self) -> (f64, f64, f64, f64) {
        ...
        (
            SLIDER_MARGIN,
            (self.logical_size.height * 0.5) - (SLIDER_HEIGHT * 0.5),
            slider_width,
            SLIDER_HEIGHT,
        )
    }
...
unsafe extern "system" fn plugview_query_interface(
    this: *mut c_void,
    iid: *const TUID,
    obj: *mut *mut c_void,
) -> tresult {
...
unsafe extern "system" fn plugview_is_platform_type_supported(
    _this: *mut c_void,
    type_: FIDString,
) -> tresult {
...
unsafe extern "system" fn plugview_attached(
    this: *mut c_void,
    parent: *mut c_void,
    type_: FIDString,
) -> tresult {
    let obj = this as *mut PlugViewObj;
    if (*obj).gui_handle.is_some() {
        return kResultOk;
    }
    ...
    let parent_handle = {
        ...
        if type_bytes.starts_with(b"NSView") {
            std::ptr::NonNull::new(parent)
                .map(|ns_view| RawWindowHandle::AppKit(AppKitWindowHandle::new(ns_view)))
        } else {
            None
        }
    };
    ...
    let parent_handle = {
        ...
        if type_bytes.starts_with(b"HWND") {
            NonZeroIsize::new(parent as isize)
    ...
    if let Some(handle) = parent_handle {
        ...
        let options = WindowOpenOptions::new(
            "Gain",
            Size::new(GUI_WIDTH as f64, GUI_HEIGHT as f64),
...
unsafe extern "system" fn plugview_on_wheel(_this: *mut c_void, _distance: f32) -> tresult {
    kResultOk
}
unsafe extern "system" fn plugview_on_key_down(
    _this: *mut c_void,
    _key: char16,
    _key_code: int16,
    _modifiers: int16,
...
unsafe extern "system" fn plugview_on_key_up(
    _this: *mut c_void,
    _key: char16,
    _key_code: int16,
    _modifiers: int16,
...
unsafe extern "system" fn plugview_on_size(
    _this: *mut c_void,
    _new_size: *mut ViewRect,
) -> tresult {
    kResultOk
}
unsafe extern "system" fn plugview_on_focus(_this: *mut c_void, _state: TBool) -> tresult {
    kResultOk
}
unsafe extern "system" fn plugview_set_frame(_this: *mut c_void, _frame: *mut c_void) -> tresult {
    kResultOk
}
unsafe extern "system" fn plugview_can_resize(_this: *mut c_void) -> tresult {
    kResultFalse
}
unsafe extern "system" fn plugview_check_size_constraint(
    _this: *mut c_void,
    _rect: *mut ViewRect,
) -> tresult {
    kResultOk
...
unsafe extern "system" fn factory_get_class_info(
    ...
    match index {
        0 => {
            ...
            strcpy_safe(&mut info.name, b"Vst3 Sys Fx Gain GUI Baseview\0");
        }
        1 => {
            ...
            strcpy_safe(
                &mut info.name,
                b"Vst3 Sys Fx Gain GUI Baseview Controller\0",
            );
        }
...
unsafe extern "system" fn factory_create_instance(
    ...
    *obj = std::ptr::null_mut();
    ...
    let instance = if iid_equal(&cid_arr, &CID_PROCESSOR) {
        GainProcessorObj::new((*factory).gain_state.clone()) as *mut c_void
    } else if iid_equal(&cid_arr, &CID_CONTROLLER) {
        GainControllerObj::new((*factory).gain_state.clone()) as *mut c_void
    } else {
        return kNoInterface;
    };
...
unsafe extern "system" fn factory_get_class_info_unicode(
    ...
    match index {
        0 => {
            ...
            info.cardinality = PClassInfo::kManyInstances;
            strcpy_safe(&mut info.category, kVstAudioEffectClass);
            str16cpy(&mut info.name, "Vst3 Sys Fx Gain GUI Baseview");
            info.class_flags = ComponentFlags::kSimpleModeSupported;
            strcpy_safe(&mut info.sub_categories, PlugType::kFx);
        ...
        1 => {
            ...
            info.cardinality = PClassInfo::kManyInstances;
            strcpy_safe(&mut info.category, kVstComponentControllerClass);
            str16cpy(&mut info.name, "Vst3 Sys Fx Gain GUI Baseview Controller");
            info.class_flags = 0;
            strcpy_safe(&mut info.sub_categories, b"\0");
            str16cpy(&mut info.vendor, "aizcutei");
            str16cpy(&mut info.version, "0.1.0");

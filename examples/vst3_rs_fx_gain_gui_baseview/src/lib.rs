...
fn get_shared_state() -> Arc<SharedState> {
    SHARED_STATE
        .get_or_init(|| {
            Arc::new(SharedState {
                gain_bits: AtomicU64::new(0.5_f64.to_bits()),
            })
        })
        .clone()
}
...
struct MyGuiGain {
    ...
    gui_handle: Option<ThreadSafeWindowHandle>,
}

struct ThreadSafeWindowHandle(WindowHandle);
...
impl GuiPlugin for MyGuiGain {
    fn gui_size() -> gui::GuiSize {
        gui::GuiSize::new(GUI_WIDTH, GUI_HEIGHT)
    }
    ...
    fn gui_create(&mut self, parent: RawWindowHandle) -> bool {
        ...
        let handle = Window::open_parented(&ParentWindow(parent), options, move |window| {
            GainGuiHandler::new(window, state)
        });
        self.gui_handle = Some(ThreadSafeWindowHandle(handle));
        true
    }
...
impl GainGuiHandler {
    ...
    fn draw(&mut self, window: &Window) {
        ...
        let (w, h) = (
            self.physical_size.width as usize,
            self.physical_size.height as usize,
        );
        if w == 0 || h == 0 {
            return;
        }
        ...
        let (sx, sy) = (
            self.physical_size.width as f64 / self.logical_size.width.max(1.0),
...
fn fill_rect(
    ...
    let (x0, y0) = (
        (r.0 * sx).round().max(0.0) as usize,
        (r.1 * sy).round().max(0.0) as usize,
    );
    let (x1, y1) = (
        ((r.0 + r.2) * sx).round().min(w as f64) as usize,
        ((r.1 + r.3) * sy).round().min(h as f64) as usize,
    );
    for row in y0..y1 {
        for col in x0..x1 {
            buf[row * w + col] = c;

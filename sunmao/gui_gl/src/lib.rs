...
impl GlContext {
    ...
    pub fn begin_frame(&mut self) {
        unsafe {
            self.gl.viewport(0, 0, (self.width * self.scale) as i32, (self.height * self.scale) as i32);
    ...
    fn set_color(&self, color: Color) {
        unsafe {
            self.gl.uniform_4_f32(Some(&self.u_color), color.r, color.g, color.b, color.a);
        }
    }
    ...
    fn set_ortho_transform(&self) {
        ...
        unsafe {
            self.gl.uniform_matrix_4_f32_slice(Some(&self.u_transform), false, &matrix);
        }
    }
    ...
    fn generate_arc_vertices(&self, cx: f32, cy: f32, radius: f32, start: f32, end: f32, segments: usize) -> Vec<f32> {
    ...
    fn generate_rounded_rect_vertices(&self, x: f32, y: f32, w: f32, h: f32, r: f32, segments: usize) -> Vec<f32> {

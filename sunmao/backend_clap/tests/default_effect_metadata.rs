...
#[derive(Default)]
struct DefaultEffectMetadataPlugin;

impl SunmaoPlugin for DefaultEffectMetadataPlugin {
    const NAME: &'static str = "Default Metadata Effect";
...
export_sunmao_clap_plugin!(DefaultEffectMetadataPlugin);
...
#[test]
fn default_effect_export_has_a_plugin_specific_id_and_effect_feature() {
    ...
    assert_eq!(
        id,
        derive_clap_id(
            DefaultEffectMetadataPlugin::VENDOR,
            DefaultEffectMetadataPlugin::NAME
        )
    );
    ...
    assert!(unsafe { *descriptor.features.add(1) }.is_null());
}
